...
describe('Level e2e test', () => {
    ...
    it('should load Levels', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="level"]')).first().click().then(() => {
            const expectVal = /Levels/;
    ...
    it('should load create Level dialog', () => {
        element(by.css('button.create-level')).click().then(() => {
            const expectVal = /Create or edit a Level/;
