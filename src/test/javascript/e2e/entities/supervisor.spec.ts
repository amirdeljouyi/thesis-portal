...
describe('Supervisor e2e test', () => {
    ...
    it('should load Supervisors', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="supervisor"]')).first().click().then(() => {
            const expectVal = /Supervisors/;
    ...
    it('should load create Supervisor dialog', () => {
        element(by.css('button.create-supervisor')).click().then(() => {
            const expectVal = /Create or edit a Supervisor/;
