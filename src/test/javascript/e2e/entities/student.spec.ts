...
describe('Student e2e test', () => {
    ...
    it('should load Students', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="student"]')).first().click().then(() => {
            const expectVal = /Students/;
    ...
    it('should load create Student dialog', () => {
        element(by.css('button.create-student')).click().then(() => {
            const expectVal = /Create or edit a Student/;
